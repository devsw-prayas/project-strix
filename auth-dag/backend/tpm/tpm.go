...
// Parent key persisted encrypted; child public metadata persisted; child private keys are memory-only.
type TPM struct {
...
// New creates an ephemeral TPM with a freshly generated parent key (not persisted).
func New() (*TPM, error) {
...
// NewWithEncryptedStorageFromEnv reads master key from FAKE_TPM_MASTER_KEY and calls NewWithEncryptedStorage.
// If the env var is absent, returns an error.
...
// NewWithEncryptedStorage loads or initializes a TPM with a persisted, AES-GCM-encrypted parent key.
// storageDir will contain parent.key.enc and a children/ directory with per-child JSON metadata.
// masterKey may be either a 32-byte key or any passphrase; it will be converted to a 32-byte key via SHA256.
...
// ParentPublic returns raw parent public key bytes.
func (t *TPM) ParentPublic() []byte {
	return t.parentPub
}

// ParentPublicB64 returns base64-encoded parent public key.
func (t *TPM) ParentPublicB64() string {
	return base64.StdEncoding.EncodeToString(t.parentPub)
}

// CreateChild creates or returns metadata for a child id.
// Behavior unchanged: if metadata exists, public key fixed; otherwise generate new keypair (private in-memory).
func (t *TPM) CreateChild(id, policy string) (string, Attestation, error) {
	...
	// if we already have the child loaded in memory (private present), return it
	...
	if exists(metaPath) {
		// load the metadata (public key + attestation) but private will remain nil (can't sign)
	...
	// No persisted metadata: create new keypair, persist public+attestation, but keep private in memory
	pub, priv, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
...
// Sign signs msg using the child's private key (must be present in-memory).
...
// GetChildInfo returns attestation for a child known to TPM (either loaded from disk or created).
...
// VerifyChain verifies attestation signature (parent -> child pub) and the child signature over msg.
// parentPub is raw bytes of parent public key.
...
// PersistParentToEncryptedPath allows persisting the current parent private key encrypted to a path.
// masterKey is passphrase or raw key bytes (will be SHA256'd to 32 bytes).
